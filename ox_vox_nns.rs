...
// High performance (hopefully) hybrid nearest neighbour search
//
//
...
fn ox_vox_nns<'py>(_py: Python<'py>, m: &'py PyModule) -> PyResult<()> {
    ...
    fn find_neighbours(
        ...
        // First: voxelise pointcloud - compute voxel coords for each point and
        // construct lookup of point indices by voxel coords
        let (voxel_indices, points_by_voxel) = voxelise_points(search_points, voxel_size);

        // Second: Generate voxel offsets for each layer of concentric cubes
        ...
        let mut indices: Array2<i32> =
            Array2::zeros([num_query_points, num_neighbours as usize]) - 1;
        ...
        query_points
            ...
            .for_each(|((query_point, mut indices_row), mut distances_row)| {
    ...
    #[pyfn(m)]
    #[pyo3(name = "find_neighbours")]
    fn find_neighbours_py<'py>(
        py: Python<'py>,
        ...
        let (point_indices, point_distances) = find_neighbours(
            search_points,
            query_points,
            num_neighbours,
            max_dist,
        ...
        (
            point_indices.into_pyarray(py),
            point_distances.into_pyarray(py),
        )
    }
